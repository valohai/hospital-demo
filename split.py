...
df_train.drop(["hospital_death"], axis=1).to_csv(get_output_path("train/data.csv"), index=False)
df_train[["encounter_id", "hospital_death"]].to_csv(get_output_path("train/labels.csv"), index=False)
df_test.drop(["hospital_death"], axis=1).to_csv(get_output_path("test/data.csv"), index=False)
